...
{
  mlx_put_image_to_window(mlx,win1,it_m3,0,0);
}

int	expose_win2(void *p)
{
  mlx_put_image_to_window(mlx,win2,it_m4,0,0);
  mlx_put_image_to_window(mlx,win2,it_m2,0,0);
}
...
int	main()
{
  ...
  printf(" => Image3 ZPixmap %dx%d ...",It_m3_SX,It_m3_SY);
  if (!(it_m3 = mlx_new_image(mlx,It_m3_SX,It_m3_SY)))
    {
      printf(" !! KO !!\n");
      exit(1);
    }
  data3 = mlx_get_data_addr(it_m3,&bpp3,&sl3,&endian3);
  printf("OK (bpp3 %d, sizeline3 %d endian3 %d type %d)\n",bpp3,sl3,endian3,
	 ((t_img *)it_m3)->type);

  printf(" => Fill Image3 ...");
  color_map_2(data3,bpp3,sl3,It_m3_SX,It_m3_SY,endian3, 1);
  printf("OK (pixmap : %d)\n",(int)((t_img *)it_m3)->pix);

  printf(" => Put Image3 ...");
  mlx_put_image_to_window(mlx,win1,it_m3,20,20);
  printf("OK\n");
  sleep(2);
  ...
  if (!(it_m2 = mlx_xpm_file_to_image(mlx,"open.xpm",&xpm1_x,&xpm1_y)))
    {
      printf(" !! KO !!\n");
      exit(1);
    }
  data2 = mlx_get_data_addr(it_m2,&bpp2,&sl2,&endian2);
  printf("OK (xpm %dx%d)(img bpp2: %d, sizeline2: %d endian: %d type: %d)\n",
	 xpm1_x,xpm1_y,bpp2,sl2,endian2,((t_img *)it_m2)->type);
  sleep(2);

  printf(" => Put xpm ...");
  mlx_put_image_to_window(mlx,win1,it_m2,0,0);
  mlx_put_image_to_window(mlx,win1,it_m2,100,100);
  printf("OK\n");
  sleep(2);

  printf(" => 2nd window,");
  win2 = mlx_new_window(mlx,WIN1_SX,WIN1_SY,"Title2");
  if (!(it_m4 = mlx_new_image(mlx,It_m3_SX, It_m3_SY)))
    {
      printf(" !! KO !!\n");
      exit(1);
    }
  data4 = mlx_get_data_addr(it_m4,&bpp4,&sl4,&endian4);
  color_map_2(data4,bpp4,sl4,It_m3_SX,It_m3_SY,endian4, 2);
