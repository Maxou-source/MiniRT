				     ...
				     int (*xpm_func)(),void *param)
{
  ...
  if (!(it_m2 = (void *)mlx_new_image(xvar,img1->width,img1->height)))
    {
      XDestroyImage(img1);
      return ((void *)0);
    }
  *width = img1->width;
  *height = img1->height;
  if (mlx_int_egal_img(it_m2->image,img1))
    {
      bcopy(img1->data,it_m2->data,img1->height*img1->bytes_per_line);
      XDestroyImage(img1);
      return (it_m2);
    }
  if (it_m2->type==MLX_TYPE_SHM_PIXMAP)
    {
      XFreePixmap(xvar->display,it_m2->pix);
      it_m2->pix = XCreatePixmap(xvar->display,xvar->root,
			       *width,*height,xvar->depth);
    }
  if (it_m2->type>MLX_TYPE_XIMAGE)
    {
      XShmDetach(xvar->display,&(it_m2->shm));
      shmdt(it_m2->data);
    }
  XDestroyImage(it_m2->image);
  it_m2->image = img1;
  it_m2->data = img1->data;
  it_m2->type = MLX_TYPE_XIMAGE;
  it_m2->size_line = img1->bytes_per_line;
  it_m2->bpp = img1->bits_per_pixel;
  return (it_m2);
}
